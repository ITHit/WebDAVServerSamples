...
(function (WebdavCommon) {
  var sOverwriteDialogueFormat =
    "The following item(s) exist on the server:<br/><br/>{0}<br/><br/>Overwrite?";
  var sFailedCheckExistsMessage =
    "Check for already exists item failed with error.";
  ...
  function UploaderGridView(sSelector) {
    ...
    this._dropZone.HtmlElement.addEventListener(
      "dragenter",
    ...
    this.Uploader.SetUploadUrl(
      ITHit.WebDAV.Client.Encoder.Decode(window.location.href.split("#")[0])
    );
    ...
    this.fileLoadCompleted = function () {
      if (this.$container.find(".uploading-item").length == 0) {
        ...
        this.$container
          .find(".progress-wrapper .progress-bar")
   ...
   */
  UploaderGridView.prototype._OnUploadItemsCreated = function (
    oUploadItemsCreated
  ) {
    ...
    this._GetExistsAsync(
      oUploadItemsCreated.Items,
      function (oAsyncResult) {
        ...
        if (!oAsyncResult.IsSuccess) {
          ...
          this._ShowExistsCheckError(oAsyncResult.Error, function () {
        ...
        oAsyncResult.Result.forEach(function (oUploadItem) {
          ...
          sItemsList += decodeURI(oUploadItem.GetRelativePath()) + "<br/>";
          aExistsUploadItems.push(oUploadItem);
        });

        /* One or more items exists on the server. Show Overwrite / Skip / Cancel dialog.*/
        oConfirmModal.Confirm(
          WebdavCommon.PasteFormat(sOverwriteDialogueFormat, sItemsList),
          ...
          function onSkipExists() {
            ...
            var aNotExistsUploadItems = $.grep(
              oUploadItemsCreated.Items,
  ...
  UploaderGridView.prototype._ValidateSize = function (oUploadItem) {
    if (oUploadItem.GetSize() > iMaxFileSize) {
      var sMessage = WebdavCommon.PasteFormat(
        sWrongFileSizeFormat,
        WebdavCommon.Formatters.FileSize(iMaxFileSize)
      );
  ...
  UploaderGridView.prototype._ValidateName = function (oUploadItem) {
    var sValidationMessage = WebdavCommon.Validators.ValidateName(
      oUploadItem.GetName()
    );
   ...
   * @memberof UploaderGridView.prototype
   */
  UploaderGridView.prototype._GetExistsAsync = function (
    aUploadItems,
    fCallback
  ) {
    this._OpenItemsCollectionAsync(aUploadItems, function (aResultCollection) {
      var oFailedResult = ITHit.Utils.FindBy(
        aResultCollection,
        function (oResult) {
          return !(
   ...
   * @memberof UploaderGridView.prototype
   */
  UploaderGridView.prototype._OpenItemsCollectionAsync = function (
    aUploadItems,
    fCallback
  ) {
    ...
    aUploadItems.forEach(function (oUploadItem) {
      window.WebDAVController.WebDavSession.OpenItemAsync(
        ITHit.EncodeURI(oUploadItem.GetUrl()),
        [],
        function (oAsyncResult) {
          iCounter--;
          aResults.push({
  ...
  UploaderGridView.prototype._QueueChange = function (oQueueChanged) {
    ...
    oQueueChanged.AddedItems.forEach(
      function (value) {
        var row = new UploaderGridRow(
          value,
          this.fileLoadCompleted.bind(this),
          this._ShowExistsCheckError.bind(this),
          this._DataBindAllProgress.bind(this),
          this._StateRowChange.bind(this)
        );
        this.rows.push(row);
    ...
    oQueueChanged.RemovedItems.forEach(
      function (value) {
        var aRows = $.grep(this.rows, function (oElem) {
          return value === oElem.UploadItem;
        });
  ...
  UploaderGridView.prototype._StateRowChange = function () {
    ...
    this.rows.forEach(function (row) {
      ...
      if (rowState === window.ITHit.WebDAV.Client.Upload.State.Paused) {
        ...
        rowState === window.ITHit.WebDAV.Client.Upload.State.Completed ||
  ...
  UploaderGridView.prototype._DataBindAllProgress = function () {
    ...
    this.rows.forEach(function (value) {
      if (
        value.UploadItem.GetState() !==
        window.ITHit.WebDAV.Client.Upload.State.Canceled
      ) {
    ...
    if (currProgress >= 0) {
      ...
      $progress
        .attr("aria-valuenow", currProgress)
        .css("width", currProgress + "%");
      this.$uploadingBlock
        .find(".persent")
  ...
  UploaderGridView.prototype._DataBindUploaderBlock = function () {
    this.$container
      .find(".pause-all-button")
      .click(this._PauseAllClickHandler.bind(this));
    this.$container
      .find(".play-all-button")
      .click(this._StartAllClickHandler.bind(this));
    this.$uploadingBlock
      .find(".details-button")
      .click(this._DetailsClickHandler.bind(this));
    this.$uploadingDetails
      .find(".close-button")
      .click(this._CloseClickHandler.bind(this));
    this.$container
  ...
  UploaderGridView.prototype._StartAllClickHandler = function () {
    ...
    this.rows.forEach(function (value) {
      if (
        value.UploadItem.GetState() ===
        window.ITHit.WebDAV.Client.Upload.State.Paused
      ) {
  ...
  UploaderGridView.prototype._ShowExistsCheckError = function (
    oError,
    fCallback
  ) {
   ...
   */
  function UploaderGridRow(
    oUploadItem,
    fileLoadCompletedCallback,
  ...
  UploaderGridRow.prototype._Render = function (oUploadItem) {
    ...
    var $itemIcon = $('<div class="col-auto px-0 file-wrapper" />').append(
      $('<div class="icon icon-file" />')
    );

    var $itemData = $('<div class="col" />')
      .append(
        $('<div class="row align-items-center" />').html(
      ...
      .append(
        $('<div class="row justify-content-between mt-1" />').html(
  ...
  UploaderGridRow.prototype._DataBindActions = function (oUploadItem) {
    if (
      oUploadItem.GetState() === window.ITHit.WebDAV.Client.Upload.State.Paused
    ) {
  ...
  UploaderGridRow.prototype._DataBind = function (oUploadItem) {
    ...
    var sFileExtansion = WebdavCommon.Formatters.GetFileExtension(
      oUploadItem.GetName()
    );
    var $itemIcon = $container.find(".icon-file");
    var oProgress = oUploadItem.GetProgress();
    if (sFileExtansion.length < 5) {
      $itemIcon.addClass("icon-file-" + sFileExtansion);
      $itemIcon.html(
        '<span class="file-extension">' +
          sFileExtansion.toUpperCase() +
          "</span>"
      );
    }
    $container
    ...
    $container
      .find(".item-size")
      .text(WebdavCommon.Formatters.FileSize(oProgress.TotalBytes));
    $container.find(".item-speed").text(oProgress.Completed + " % done");
    $container
      .find(".item-progress")
      .text(WebdavCommon.Formatters.FileSize(oProgress.Speed) + "/sec");

    this._DataBindActions(oUploadItem);
    var sCurrentState = oUploadItem.GetState();
    if (
      sCurrentState === window.ITHit.WebDAV.Client.Upload.State.Completed ||
  ...
  UploaderGridRow.prototype._DataBindProgressRow = function (oUploadItem) {
    ...
    this.$el
      .find(".progress-bar")
      .attr("aria-valuenow", oProgress.Completed)
      .css("width", oProgress.Completed + "%");
   ...
   */
  UploaderGridRow.prototype._OnBeforeUploadStarted = function (
    oBeforeUploadStarted
  ) {
    ...
    window.WebDAVController.WebDavSession.OpenItemAsync(
      ...
      function (oAsyncResult) {
        ...
        if (!oAsyncResult.IsSuccess) {
          ...
          this.fileUploadFailedCallback(oAsyncResult.Error, function () {
  ...
  UploaderGridRow.prototype._SetRetryMessage = function (timeLeft) {
    var sMessage = WebdavCommon.PasteFormat(
      sRetryMessageFormat,
      WebdavCommon.Formatters.TimeSpan(Math.ceil(timeLeft / 1000))
    );
    this.$el
      .find(".retry-message")
      .html(sMessage)
...
export const WebDAVUploaderGridView = window.WebDAVUploaderGridView;
