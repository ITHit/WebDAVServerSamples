...
import { WebDAVUploaderGridView } from "./webdav-uploader";
...
(function (WebdavCommon) {
  ...
  var FolderGridView = function (selectorTableContainer, selectorTableToolbar) {
    ...
    this.$el.on(
      {
        mouseenter: function () {
          ...
          else $(this).addClass("hover").next().addClass("hover");
        },
        mouseleave: function () {
          if ($(this).hasClass("tr-snippet-url"))
            $(this).removeClass("hover").prev().removeClass("hover");
          else $(this).removeClass("hover").next().removeClass("hover");
        },
      },
      "tr"
    );
    ...
    setInterval(function () {
      self.$el.find("td.modified-date").each(function () {
        $(this).text(
          WebdavCommon.Formatters.Date($(this).data("modified-date"))
        );
      });
    }, 3000);
  };
  FolderGridView.prototype = {
    Render: function (aItems, bisSearchMode) {
      ...
      this.$el.find("tbody").html(
        aItems.map(
          function (oItem) {
            var locked =
              oItem.ActiveLocks.length > 0
                ? '<span class="icon icon-locked" title="' +
            ...
            return $("<div/>")
              .html([
                $("<tr />")
                  .html([
                    $('<td class="select-disabled"/>').html([$customCheckbox]),
                    $("<td />").html(
                      oItem.IsFolder()
                        ? '<span class="icon icon-folder">' + locked + "</span>"
                        : locked
                    ),
                    this._RenderDisplayName(oItem),
                    $('<td class="d-none d-xl-table-cell" />').text(
                      oItem.IsFolder()
                        ? "Folder"
                        : "File " +
                            WebdavCommon.Formatters.GetFileExtension(
                              oItem.DisplayName
                            )
                    ),
                    $("<td />")
                      .text(
                        !oItem.IsFolder()
                          ? WebdavCommon.Formatters.FileSize(
                              oItem.ContentLength
                            )
                          : ""
                      )
                      .css("text-align", "right"),
                    $('<td class="d-none d-lg-table-cell modified-date" />')
                      .text(WebdavCommon.Formatters.Date(oItem.LastModified))
                      .data("modified-date", oItem.LastModified),
                    $('<td class="text-right select-disabled"/>').html(
                      this._RenderActions(oItem)
                    ),
                  ])
                  .on("click", function (e) {
                    ...
                    if (
                      ...
                      ((e.target.nodeName.toLowerCase() === "td" &&
                        !$(e.target).hasClass("select-disabled")) ||
                        (e.target.nodeName.toLowerCase() !== "td" &&
                          !$(e.target)
                            .parents("td")
                            .hasClass("select-disabled")))
                    ) {
                      $(this)
                        .addClass("active")
                        .siblings()
                  ...
                  .addClass(
                    self.selectedItem != null &&
                      oItem.Href == self.selectedItem.Href
                      ? "active"
                      : ""
                  ),
                $('<tr class="tr-snippet-url"/>').html([
                  ...
                  this._RenderSnippetAndUrl(oItem),
                ]),
              ])
              .children();
          }.bind(this)
    ...
    _RenderLokedIconTooltip(oItem) {
      ...
      if (oItem.ActiveLocks[0].LockScope === "Shared") {
        var userNames = [];
        tooltipTitle =
          "Shared lock" + (oItem.ActiveLocks.length > 1 ? "(s)" : "") + ": ";
    ...
    _RenderDisplayName: function (oItem) {
      var oElement = oItem.IsFolder()
        ? $('<td class="ellipsis" />').html(
            $("<a />")
              .text(oItem.DisplayName)
              .attr("href", WebdavCommon.GetApplicationUrl(oItem.Href))
          )
        : $('<td class="ellipsis" />').html(
            $("<span />").text(oItem.DisplayName)
          );
    ...
    _RenderSnippetAndUrl: function (oItem) {
      ...
      var $nav = $("<nav />").addClass("snippet-breadcrumb").appendTo(oElement);
      var $bcContainer = $("<div />")
        .addClass("ithit-breadcrumb-container")
        .appendTo($nav);
      // Append path on search mode
      if (this.isSearchMode) {
        new BreadcrumbsView(
          $("<ol />").addClass("breadcrumb").appendTo($bcContainer)
        ).SetHierarchyItem(oItem);

        $bcContainer.find("ol li:first").hide();

        // Append snippet to name
        oElement.append(
          WebdavCommon.Formatters.Snippet(
            oItem.Properties.Find(oWebDAV.SnippetPropertyName)
          )
        );
      }
    ...
    _RenderActions: function (oItem) {
      var self = this;
      var actions = [];
      var isDavProtocolSupported =
        ITHit.WebDAV.Client.DocManager.IsDavProtocolSupported();
      var isMicrosoftOfficeDocument =
        ITHit.WebDAV.Client.DocManager.IsMicrosoftOfficeDocument(oItem.Href);
      var isGSuiteDocument = ITHit.WebDAV.Client.DocManager.IsGSuiteDocument(
        oItem.Href
      );
      ...
      if (oItem.IsFolder()) {
        actions.push(
          $(
            ...
            .html(
              '<span class="btn-label"><i class="icon icon-open-folder"></i></span> <span class="d-none d-lg-inline-block">Browse</span>'
            )
        ...
        var isExclusiveLocked =
          oItem.ActiveLocks.length > 0 &&
          oItem.ActiveLocks[0].LockScope === "Exclusive";
        $(
          '<button type="button" class="btn btn-primary btn-sm btn-labeled btn-default-edit" title="' +
            (displayRadioBtns
              ? self._GetActionGroupBtnTooltipText()
              : "Edit document with desktop associated application.") +
            '"' +
            self._GetDisabledGroupBtnAttribute(isExclusiveLocked) +
            '><span class="btn-label"><i class="icon ' +
            (displayRadioBtns
              ? self._GetActionGroupBtnCssClass()
              : "icon-edit") +
            '"></i></span><span class="d-none d-lg-inline-block btn-edit-label">Edit</span></button>'
        )
          .appendTo($btnGroup)
          .on("click", function () {
          ...
          .prop(
            "disabled",
            !isDavProtocolSupported && !isMicrosoftOfficeDocument
          );
    ...
    _RenderContextMenu: function (
      ...
      var self = this;
      var supportGSuiteFeature =
        oWebDAV.OptionsInfo.Features & ITHit.WebDAV.Client.Features.GSuite;
      var displayRadioBtns =
        isMicrosoftOfficeDocument && isGSuiteDocument && process.env.ISGSUITE;
      var $dropdownMenu = $(
        '<div class="dropdown-menu dropdown-menu-right actions ' +
          (displayRadioBtns ? "dropdown-menu-radio-btns" : "") +
          '"></div>'
      ).appendTo($btnGroup);
      if (isMicrosoftOfficeDocument) {
        if (displayRadioBtns) {
          $(
            '<label class="custom-radiobtn"><input type="radio" name="defaultEditor' +
              oItem.DisplayName +
              '" value="OSEditor" ' +
              self._GetContextMenuRadioBtnCheckedProperty("OSEditor") +
              ' /><span class="checkmark"></span></label>'
          )
            .appendTo($dropdownMenu)
            .find("input[type=radio]")
            .change(function () {
              self._ChangeContextMenuRadionBtnHandler($(this));
            });
        }
        $(
          '<a class="dropdown-item ' +
            (displayRadioBtns ? "dropdown-radio" : "") +
            '" href="javascript:void()" title="Edit with Microsoft Office Desktop."><i class="icon ' +
            (displayRadioBtns
              ? "icon-action-microsoft-edit"
              : "icon-edit-ms-associated") +
            '"></i>Edit with Microsoft Office Desktop</a>'
        )
          .appendTo($dropdownMenu)
          .on("click", function () {
            oWebDAV.EditDoc(oItem.Href);
      ...
      if (isGSuiteDocument && process.env.ISGSUITE) {
        if (displayRadioBtns) {
          $(
            '<label class="custom-radiobtn"><input type="radio" name="defaultEditor' +
              oItem.DisplayName +
              '" value="GSuiteEditor" ' +
              self._GetContextMenuRadioBtnCheckedProperty("GSuiteEditor") +
              '/><span class="checkmark"></span></label>'
          )
            .appendTo($dropdownMenu)
            .find("input[type=radio]")
            ...
            .attr("disabled", !supportGSuiteFeature);
        }
        $(
          '<a class="dropdown-item ' +
            (displayRadioBtns ? "dropdown-radio" : "") +
      ...
      if (!isMicrosoftOfficeDocument) {
        $(
          '<a class="dropdown-item' +
            (isDavProtocolSupported ? "" : " disabled") +
            '" href="javascript:void()" title="Edit document with desktop associated application."><i class="icon icon-edit-associated"></i>Edit with Associated Desktop Application</a>'
        )
          .appendTo($dropdownMenu)
          .on("click", function () {
            oWebDAV.EditDoc(oItem.Href);
      ...
      $(
        '<a class="dropdown-item desktop-app' +
          (isDavProtocolSupported ? "" : " disabled") +
          '" href="javascript:void()" title="Select application to open this file with.">Select Desktop Application</a>'
      )
        .appendTo($dropdownMenu)
        .on("click", function () {
    ...
    _ChangeContextMenuRadionBtnHandler: function ($radioBtn) {
      ...
      $(".btn-default-edit").each(function () {
        ...
        if ($btn.parent().find(".actions input[type=radio]").length) {
          ...
          if (
            self._defaultEditor == "GSuiteEditor" &&
            $btn.parent().find(".dropdown-menu-radio-btns a.disabled").length
          ) {
  ...
  SearchFormView.prototype = {
    ...
    SetDisabled: function (bIsDisabled) {
      ...
      this.$el
        ...
        .attr(
          "placeholder",
          !bIsDisabled ? "" : "The server does not support search"
        );
    },
    ...
    LoadFromHash: function () {
      ...
      this._RenderFolderGrid(
        oWebDAV.GetHashValue("search"),
        oWebDAV.GetHashValue("page")
      );
    },

    _Source: function (sPhrase, c, fCallback) {
      oWebDAV.NavigateSearch(
        sPhrase,
        false,
        1,
        false,
    ...
    _RenderFolderGrid: function (oSearchQuery, nPageNumber) {
      ...
      oWebDAV.NavigateSearch(
        ...
        function (oResult) {
          ...
          oPagination.Render(
            nPageNumber,
            Math.ceil(oResult.Result.TotalItems / oWebDAV.PageSize),
            function (pageNumber) {
    ...
    _RenderSuggestion: function (oItem) {
      ...
      // Append path
      new BreadcrumbsView(
        $("<ol />").addClass("breadcrumb").appendTo(oElement)
      ).SetHierarchyItem(oItem);
  ...
  BreadcrumbsView.prototype = {
    ...
    SetHierarchyItem: function (oItem) {
      var sUrl = WebdavCommon.SliceLastSymbol(
        WebdavCommon.GetTail(
          WebdavCommon.GetWebDavServerUrl(oItem.Href),
          WebdavCommon.GetWebDavServerUrl()
        ),
        "/"
      );
      aParts = [WebdavCommon.GetWebDavServerUrl()];
      var aParts = aParts.concat(
        sUrl
          .split("/")
          .slice(1)
          .filter(function (v) {
      ...
      this.$el.html(
        aParts.map(function (sPart, i) {
          var bIsLast = aParts.length === i + 1;
          var oLabel =
            i === 0
              ? $("<span />").addClass("icon icon-home")
              : $("<span />").text(decodeURIComponent(sPart));
          return $('<li class="breadcrumb-item"/>')
            .toggleClass("active", bIsLast)
            .append(
              bIsLast
                ? $("<span />").html(oLabel)
                : $("<a />")
                    .attr("href", aParts.slice(0, i + 1).join("/") + "/")
                    .html(oLabel)
            );
        })
      );
    ...
    Render: function (pageNumber, countPages, changePageCallback) {
      ...
      if (countPages && countPages > 1) {
        ...
        $("<a />")
          ...
          .appendTo(
            $("<li/>")
              .addClass("page-item " + (pageNumber == 1 ? "disabled" : ""))
              .appendTo(this.$el)
          )
          .text("<<")
          .click(function () {
            if (pageNumber != 1) changePageCallback(pageNumber - 1);
            return false;
          });

        // render pages
        var firstPage =
          countPages > this.maxItems &&
          pageNumber - Math.floor(this.maxItems / 2) > 0
            ? pageNumber - Math.floor(this.maxItems / 2)
            : 1;
        var lastPage =
          firstPage + this.maxItems - 1 <= countPages
            ? firstPage + this.maxItems - 1
            : countPages;

        if (
          countPages > this.maxItems &&
          lastPage - firstPage < this.maxItems
        ) {
        ...
        if (firstPage > 1 && countPages > this.maxItems) {
          $("<a />")
            ...
            .appendTo(
              $("<li/>")
                .addClass("page-item " + (1 == pageNumber ? "active" : ""))
                .appendTo(this.$el)
            )
            .text(1)
            .click(function () {
          ...
          if (firstPage - 1 > 1) {
            $("<a />")
              ...
              .data("page-number", i)
              .appendTo(
                $("<li/>").addClass("page-item disabled").appendTo(this.$el)
              )
              .text("...");
          }
        }
        ...
        for (var i = firstPage; i <= lastPage; i++) {
          $("<a />")
            ...
            .appendTo(
              $("<li/>")
                .addClass("page-item " + (i == pageNumber ? "active" : ""))
                .appendTo(this.$el)
            )
            .text(i)
            .click(function () {
        ...
        if (lastPage != countPages && countPages > this.maxItems) {
          ...
          $("<a />")
            .addClass("page-link")
            .data("page-number", countPages)
            .appendTo(
              $("<li/>")
                .addClass(
                  "page-item " + (countPages == pageNumber ? "active" : "")
                )
                .appendTo(this.$el)
            )
            .text(countPages)
        ...
        $("<a />")
          ...
          .appendTo(
            $("<li/>")
              .addClass(
                "page-item " + (countPages == pageNumber ? "disabled" : "")
              )
              .appendTo(this.$el)
          )
          .text(">>")
          .click(function () {
            if (pageNumber != countPages) changePageCallback(pageNumber + 1);
            return false;
          });
      }
    },
  ...
  TableSortingView.prototype = {
    ...
    Set: function (sortColumn, sortAscending) {
      var $col = this.$headerCols.filter(
  ...
  HistoryApiController.prototype = {
    ...
    _OnPopState: function (oEvent) {
      if (oWebDAV.GetHashValue("search")) {
        ...
        var sUrl =
          (oEvent.state && oEvent.state.Url) ||
          window.location.href.split("#")[0];
        oWebDAV.NavigateFolder(
          WebdavCommon.GetWebDavServerUrl(sUrl),
          null,
          null,
          null,
          true
    ...
    _OnLinkClick: function (oEvent) {
      ...
      if (
        sUrl.indexOf(
          location.origin ||
            window.location.href.split("#")[0].replace(location.pathname, "")
        ) !== 0
      ) {
        return;
      }
  ...
  var WebDAVController = function () {
    ...
    this.SnippetPropertyName = new ITHit.WebDAV.Client.PropertyName(
      "snippet",
      "ithit"
    );
  };
  ...
  WebDAVController.prototype = {
    ...
    NavigateFolder: function (
      ...
      var sortColumns = [
        new ITHit.WebDAV.Client.OrderProperty(
          new ITHit.WebDAV.Client.PropertyName(
            "is-directory",
            ITHit.WebDAV.Client.DavConstants.NamespaceUri
          ),
          this.CurrentSortColumnAscending
        ),
      ];
      ...
      this.WebDavSession.OpenFolderAsync(
        sPath,
        [],
        function (oResponse) {
          var self = this;
          if (oResponse.IsSuccess) {
            ...
            this.CurrentFolder.GetSupportedFeaturesAsync(function (oResult) {
              ...
              if (oResult.IsSuccess) {
                ...
                oSearchForm.SetDisabled(
                  !(
                    self.OptionsInfo.Features &
                    ITHit.WebDAV.Client.Features.Dasl
                  )
                );
              } else {
                WebdavCommon.ErrorModal.Show(
                  sSupportedFeaturesErrorMessage,
                  oResult.Error
                );
              }
            });

            this.CurrentFolder.GetPageAsync(
              [],
              ...
              function (oResult) {
                ...
                if (oResult.IsSuccess) {
                  ...
                  var aCountPages = Math.ceil(
                    oResult.Result.TotalItems / pageSize
                  );
                  ...
                  if (fCallback) fCallback(aItems);
                } else {
                  WebdavCommon.ErrorModal.Show(
                    sProfindErrorMessage,
                    oResult.Error
                  );
                }
              }
    ...
    NavigateSearch: function (
      ...
      searchQuery.SelectProperties = [this.SnippetPropertyName];

      function _getSearchPageByQuery() {
        oWebDAV.CurrentFolder.GetSearchPageByQueryAsync(
          searchQuery,
          (currentPageNumber - 1) * pageSize,
          pageSize,
          function (oResult) {
      ...
      if (
        WebdavCommon.GetWebDavServerUrl(window.location.href.split("#")[0]) !=
        this.CurrentFolder.Href
      ) {
        this.WebDavSession.OpenFolderAsync(
          WebdavCommon.GetWebDavServerUrl(window.location.href.split("#")[0]),
          [],
    ...
    EditDoc: function (sDocumentUrl) {
      if (
        ["cookies", "ms-ofba"].indexOf(
          webDavSettings.EditDocAuth.Authentication.toLowerCase()
        ) != -1
      ) {
        if (
          webDavSettings.EditDocAuth.Authentication.toLowerCase() ==
          ...
          ITHit.WebDAV.Client.DocManager.EditDocument(
            sDocumentUrl,
            this.GetMountUrl(),
            this._ProtocolInstallMessage.bind(this)
          );
        } else {
    ...
    GSuiteEditDoc: function (sDocumentUrl, gSuiteEditPanel, errorCallback) {
      ITHit.WebDAV.Client.DocManager.GSuiteEditDocument(
        sDocumentUrl,
        gSuiteEditPanel,
        errorCallback
      );
     ...
     * @param {function} [errorCallback] Function to call if document opening failed.
     */
    GSuitePreviewDoc: function (
      sDocumentUrl,
      gSuitePreviewPanel,
      errorCallback
    ) {
      ITHit.WebDAV.Client.DocManager.GSuitePreviewDocument(
        sDocumentUrl,
        gSuitePreviewPanel,
        errorCallback
      );
    ...
    GetMountUrl: function () {
      return WebdavCommon.GetWebDavServerUrl();
    },
    ...
    SetHashValues: function (arrayValues) {
      ...
      for (var i = 0; i < arrayValues.length; i++) {
        hashConfig = this._addParameterToArray(
          arrayValues[i].Name,
          arrayValues[i].Value,
          hashConfig
        );
      ...
      if (ITHit.WebDAV.Client.DocManager.IsDavProtocolSupported()) {
        ...
        if ($currentBrowser.children().length === 0) {
          ...
          if (isChrome && navigator.userAgent.indexOf("Edg") !== -1) {
            $("#DownloadProtocolModal .edge-chromium").appendTo(
              $currentBrowser
            );
          } else if (isChrome) {
            $("#DownloadProtocolModal .goole-chrome").appendTo($currentBrowser);
          } else if (typeof InstallTrigger !== "undefined") {
            $("#DownloadProtocolModal .mozilla-firefox").appendTo(
              $currentBrowser
            );
          } else if (
  ...
  var oFolderGrid = new FolderGridView(
  ...
  var oBreadcrumbs = new BreadcrumbsView(
    ".ithit-breadcrumb-container .breadcrumb",
    ".btn-up-one-level"
  );
  ...
  var oHistoryApi = new HistoryApiController(
    ".ithit-grid-container, .ithit-breadcrumb-container"
  );
  ...
  if (ITHit.WebDAV.Client.DocManager.IsDavProtocolSupported()) {
    $(".ithit-version-value").html(
      "v" +
        ...
        ' (<a href="' +
        oWebDAV.GetInstallerFileUrl() +
        '">Protocol v' +
        ITHit.WebDAV.Client.WebDavSession.ProtocolVersion +
        "</a>)"
    );
  } else {
    $(".ithit-version-value").text(
      "v" +
        ITHit.WebDAV.Client.WebDavSession.Version +
        " (Protocol v" +
        ITHit.WebDAV.Client.WebDavSession.ProtocolVersion +
  ...
  // Set protocol links
  let $protocolLinks = $(".js-protocol-link");
  $protocolLinks.attr(
    "href",
    webDavSettings.ApplicationProtocolsPath +
      $protocolLinks.attr("data-filename")
  );
})(WebdavCommon);
