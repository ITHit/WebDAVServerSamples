...
import { GSuiteEditor, Spliter } from "./webdav-gsuite";

/**
 * @namespace WebdavCommon
 */
export const WebdavCommon = (window.WebdavCommon = (function () {
  var sFileNameSpecialCharactersRestrictionFormat =
    "The name cannot contain any of the following characters: {0}";
  ...
  ns.SliceLastSymbol = function sliceLastSymbol(str, symbol) {
    var strVal = str;
    if (strVal) {
  ...
  ns.GetWebDavServerUrl = function getWebDavServerUrl(sUrl) {
    if (sUrl) {
      var resUrl = sUrl;
      var oUrl = new URL(sUrl);
      if (webDavSettings.WebDavServerPath) {
        let serverUrl = WebdavCommon.SliceLastSymbol(
          webDavSettings.WebDavServerPath,
          "/"
        );
        resUrl = WebdavCommon.SliceLastSymbol(
          serverUrl + WebdavCommon.GetTail(sUrl, serverUrl),
          "/"
        );
      }
      return resUrl;
    } else {
      return webDavSettings.WebDavServerPath
        ? WebdavCommon.SliceLastSymbol(webDavSettings.WebDavServerPath, "/")
        : location.origin;
    }
  };

  ns.GetApplicationUrl = function getApplicationUrl(sUrl, clearQuery = false) {
    var appUrl = sUrl;
    var oUrl = new URL(sUrl);

    if (oUrl.origin !== window.location.origin) {
      appUrl = sUrl.replace(
        WebdavCommon.SliceLastSymbol(webDavSettings.WebDavServerPath, "/"),
    ...
    if (clearQuery) {
      oUrl = new URL(appUrl);
      appUrl = oUrl.origin + oUrl.pathname;
    }
    return appUrl;
  };
  ...
  ns.GetTail = function getTail(sUrl1, sUrl2) {
    return sUrl1.includes(sUrl2) ? sUrl1.replace(sUrl2, "") : "";
  };
