...
const sCutItemsSameNameErrorMessage =
...
HerarhyItemsCopyPasteController.prototype = {
  ...
  Copy: function (oItem, oItemName, fCallback) {
    oItem.CopyToAsync(
      this.Toolbar.WebDAV.CurrentFolder,
      oItemName,
      true,
      null,
      null,
        ...
        self.Move(self.storedItems[index], function (oAsyncResult) {
          if (!oAsyncResult.IsSuccess) {
            if (
              ...
              WebdavCommon.ErrorModal.Show(
                sCutItemsLockedErrorMessage,
                oAsyncResult.Error
              );
            } else {
              WebdavCommon.ErrorModal.Show(
                sCutItemsErrorMessage,
                oAsyncResult.Error
              );
            }
          }
    ...
    self.Copy(oItem, oItemName, function (oAsyncResult) {
      if (!oAsyncResult.IsSuccess) {
        if (
          ...
          oAsyncResult.Error instanceof
            window.ITHit.WebDAV.Client.Exceptions.ForbiddenException
        ) {
          self._DoCopy(oItem, self._GetCopySuffix(oItemName, true));
        } else {
          WebdavCommon.ErrorModal.Show(
            sCopyItemsErrorMessage,
            oAsyncResult.Error
          );
        }
      }
  ...
  _GetCopySuffix: function (oItemName, bWithCopySuffix) {
    ...
    var sName =
      aExtensionMatches !== null
        ? oItemName.replace(aExtensionMatches[0], "")
        : oItemName;
...
export function CopyPasteButtonsControl(toolbar) {
  this.CopyButton = new BaseButton(
    "Copy",
    '<i class="icon  icon-copy-items"></i>'
  );
  this.PasteButton = new BaseButton(
    "Paste",
    '<i class="icon  icon-paste-items"></i>'
  );
  this.CutButton = new BaseButton(
    "Cut",
    '<i class="icon  icon-cut-items"></i>'
  );
  this.storedItems = [];

  var oHerarhyItemsCopyPasteController = new HerarhyItemsCopyPasteController(
    toolbar,
    this.storedItems
  );
