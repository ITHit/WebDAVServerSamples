...
namespace CalDAVServer.FileSystemStorage.AspNetCore
{
    ...
    internal class MyCustomGetHandler : IMethodHandler<IHierarchyItem>
    {
        ...
        public async Task ProcessRequestAsync(ContextAsync<IHierarchyItem> context, IHierarchyItem item)
        {
            ...
            if (item is IItemCollection)
            {
                ...
                using (TextReader reader = File.OpenText(filePath))
                {
                    ...
                    html = html.Replace("_CALENDARS_", await AllUserCalendarsUrlAsync(context));
        ...
        /// <summary>
        /// Gets all user calendars URLs.
        /// </summary>
        private async Task<string> AllUserCalendarsUrlAsync(ContextAsync<IHierarchyItem> context)
        {
            ...
            foreach (IItemCollection folder in await discovery.GetCalendarHomeSetAsync())
            {
                IEnumerable<IHierarchyItem> children = (await folder.GetChildrenAsync(new PropertyName[0], null, null, null)).Page;
                items.AddRange(children.Where(x => x is ICalendarFolder));
            }
