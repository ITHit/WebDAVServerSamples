...
namespace HttpListenerLibrary.ExtendedAttributes
{
    /// <summary>
    /// Provides methods for reading and writing extended attributes on files and folders on Linux.
    /// </summary>
    public class AndroidExtendedAttribute : IExtendedAttribute
    {
        /// <summary>
        ...
        public async Task<bool> IsExtendedAttributesSupportedAsync(string path)
        {
            ...
            int attributeCount = ListXAattr(path, null, 0);
        ...
        public async Task<string> GetExtendedAttributeAsync(string path, string attribName)
        {
            ...
            int attributeSize = GetXAttr(path, userAttributeName, null, 0);
            if (attributeSize == -1)
            {
            ...
            int readedLength = GetXAttr(path, userAttributeName, buffer, attributeSize);
        ...
        public async Task SetExtendedAttributeAsync(string path, string attribName, string attribValue)
        {
            ...
            int result = SetXAttr(path, userAttributeName, buffer, buffer.Length, 0);
        ...
        public async Task DeleteExtendedAttributeAsync(string path, string attribName)
        {
            ...
            string userAttributeName = string.Format(attributeNameFormat, attribName);
            int result = RemoveXAttr(path, userAttributeName);
        ...
        extern static private int GetXAttr(string filePath, string attribName, byte[] buffer, int bufferSize);
        ...
        extern static private int SetXAttr(string filePath, string attribName, byte[] attribValue, int size, int flags);
        ...
        extern static private int RemoveXAttr(string path, string attribName);
        ...
        extern static private int ListXAattr(string filePath, StringBuilder nameBuffer, int size);
