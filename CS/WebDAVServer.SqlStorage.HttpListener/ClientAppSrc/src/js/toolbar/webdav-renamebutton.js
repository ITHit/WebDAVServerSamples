...
RenameItemController.prototype = {
  ...
  Rename: function (newItemName, fCallback) {
    this.Toolbar.FolderGrid.selectedItems[0].MoveToAsync(
          ...
          if (!oAsyncResult.IsSuccess) {
            if (
              ...
              WebdavCommon.ErrorModal.Show(
                sRenameItemLockedErrorMessage,
                oAsyncResult.Error
              );
            } else {
              WebdavCommon.ErrorModal.Show(
                sRenameItemErrorMessage,
                oAsyncResult.Error
              );
            }
          }
...
export function ToolbarRenameButton(name, toolbar) {
  BaseButton.call(this, name, '<i class="icon  icon-rename-item"></i>');

  var oRenameItemModal = new RenameItemModal(
    "#RenameItemModal",
    new RenameItemController(toolbar)
  );
  this.Render = function () {
    this.$Button.on("click", function () {
      if (toolbar.FolderGrid.selectedItems.length) {
        oRenameItemModal.$txt.val(
          toolbar.FolderGrid.selectedItems[0].DisplayName
        );
        oRenameItemModal.oldItemName =
          toolbar.FolderGrid.selectedItems[0].DisplayName;
      }
