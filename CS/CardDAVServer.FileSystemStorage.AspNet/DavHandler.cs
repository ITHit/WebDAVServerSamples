...
namespace CardDAVServer.FileSystemStorage.AspNet
{
    ...
    public class DavHandler : HttpTaskAsyncHandler
    {
        ...
        static DavHandler()
        {
            //create serialization assemblies here.
            //Otherwise it fails with permissions error on first access in impersonated context.
            using (MemoryStream ms = new MemoryStream())
            {
                new XmlSerializer(typeof(List<PropertyValue>)).Serialize(ms, new List<PropertyValue>());
                new XmlSerializer(typeof(long)).Serialize(ms, 0L);
                new XmlSerializer(typeof(int)).Serialize(ms, 0);
            }
        }
        ...
        private DavEngineAsync initializeEngine(HttpContext context)
        {
            ...
            ILogger logger = CardDAVServer.FileSystemStorage.AspNet.Logger.Instance;
