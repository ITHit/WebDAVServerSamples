    ...
    public class CalendarFile : DavHierarchyItem, ICalendarFile
    {
        ...
        private async Task ReadAttachmentsAsync(DavContext context, IPropertyList<IMedia> attachments, IEnumerable<DataRow> rowsAttachments, ICalendar2 cal)
        {
            ...
            if (loadContent)
            {
                ...
                await using (SqlDataReader reader = await context.ExecuteReaderAsync(CommandBehavior.SequentialAccess, sql, "@EventComponentId", eventComponentId))                
                {
                    while(await reader.ReadAsync())
                    {
                        ...
                        {
                            await using (Stream stream = reader.GetStream(ordContent))
                            {
