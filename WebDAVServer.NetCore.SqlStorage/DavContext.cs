...

{
    ...
    /// </summary>
    public class DavContext : DavContextCoreBaseAsync, IDisposable
    {
        /// <summary>
        /// Context options.
        /// </summary>
        public DavContextOptions ContextOptions { get; private set; }
        ...
        private async Task<IHierarchyItemAsync> getItemByPathAsync(string path)
        {
            ...
            string command =
                  @"SELECT
                     ...
                     FROM Item
                  WHERE Name = @Name AND ParentItemId = @Parent";
            var davHierarchyItems = await ExecuteItemAsync<DavHierarchyItem>(
        ...
        /// <returns>Instance of <see cref="IHierarchyItemAsync"/>.</returns>
        private async Task<IHierarchyItemAsync> getRootFolderAsync()
        {
            string command =
               @"SELECT 
                      ItemId
                    ...
                    FROM Item
                 WHERE ItemId = @ItemId";
            var hierarchyItems = await ExecuteItemAsync<IHierarchyItemAsync>(
                "",
                command,
                "@ItemId", rootId);
