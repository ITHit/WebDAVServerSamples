...
using WebDAVServer.NetCore.FileSystem.ExtendedAttributes;
...
namespace WebDAVServer.NetCore.FileSystem
{
    ...
    public class DavFile : DavHierarchyItem, IFileAsync, IResumableUploadAsync, IUploadProgressAsync
    {
        ...
        private bool ContainsDownloadParam
        {
            get { return context.HttpContext.Request.Query.Any(x => x.Key.StartsWith("download")); }
        }
        ...
        public static async Task<DavFile> GetFileAsync(DavContext context, string path)
        {
            ...
            int serialNumber = await file.GetExtendedAttributeAsync<int?>("SerialNumber") ?? 0;
            long totalContentLength = await file.GetExtendedAttributeAsync<long?>("TotalContentLength") ?? 0;

            return new DavFile(file, context, path, serialNumber, totalContentLength);
        }
        ...
        /// <param name="context">WebDAV Context.</param>
        /// <param name="path">Encoded path relative to WebDAV root folder.</param>
        /// <param name="serialNumber">Value that is updated every time this file is updated. Used to form Etag.</param>
        /// <param name="totalContentLength">Total length of the file being uploaded. Used during uload only.</param>
        protected DavFile(FileInfo file, DavContext context, string path, int serialNumber, long totalContentLength)
        ...
        public async Task ReadAsync(Stream output, long startIndex, long count)
        {
            ...
            using (FileStream fileStream = fileInfo.Open(FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                ...
                bool responceStreamDisposed = false;
                long readedBytesCount = 0;

                while (readedBytesCount < count && !responceStreamDisposed && output.CanWrite)
                {
                    int toRead = (int)Math.Min(count - readedBytesCount, bufSize);
                    int bytesRead = await fileStream.ReadAsync(buffer, 0, toRead);
        ...
        public async Task<bool> WriteAsync(Stream content, string contentType, long startIndex, long totalFileSize)
        {
            ...
            using (var fileStream = fileInfo.Open(FileMode.OpenOrCreate, FileAccess.Write, FileShare.Read))
            {
                ...
                try
                {
                    while (content.CanRead && (lastBytesRead = await content.ReadAsync(buffer, 0, bufSize)) > 0)
                    {
        ...
        public override async Task MoveToAsync(IItemCollectionAsync destFolder, string destName, MultistatusException multistatus)
        {
            ...
            var targetFolder = (DavFolder)destFolder;
            ...
            var newDirPath = System.IO.Path.Combine(targetFolder.FullPath, destName);
