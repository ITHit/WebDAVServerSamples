...
namespace WebDAVServer.NetCore.FileSystem.ExtendedAttributes
{
    /// <summary>
    ...
    public class WindowsExtendedAttribute : IExtendedAttribute
    {
        ...
        public async Task<bool> IsExtendedAttributesSupportedAsync(string path)
        {
            ...
            if (!GetVolumeInformation(GetWin32LongPath(path), volumeName, volumeName.Capacity
        ...
        public async Task<string> GetExtendedAttributeAsync(string path, string attribName)
        {
            ...
            using (SafeFileHandle safeHandler = GetSafeHandler(fullPath, FileAccess.Read, FileMode.Open, FileShare.ReadWrite))
            {
                if (safeHandler.IsInvalid)
                {
                    ...
                    if (error == 2) // File or alternate stream not found.
                    {
                        return null;
                    }
                }
