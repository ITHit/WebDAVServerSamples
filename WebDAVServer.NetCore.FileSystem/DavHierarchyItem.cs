...
namespace WebDAVServer.NetCore.FileSystem
{
    ...
    public abstract class DavHierarchyItem : IHierarchyItemAsync, ILockAsync, IMsItemAsync
    {
        ...
        /// <summary>
        /// Retrieves full path for this file/folder in file system.
        /// </summary>
        ...
        public async Task<IEnumerable<PropertyValue>> GetPropertiesAsync(IList<PropertyName> props, bool allprop)
        {
            ...
            if (!allprop)
            {
                propertyValues = propertyValues.Where(property => props.Contains(property.QualifiedName)).ToList();
            }
        ...
        {
            var propertyValues = await GetPropertyValuesAsync();
        ...
        public async Task UnlockAsync(string lockToken)
        {
            ...
            var locks = await GetLocksAsync(getAllWithExpired: true);
        ...
        private async Task<List<DateLockInfo>> GetLocksAsync(bool getAllWithExpired = false)
        {
            ...
            {
                var resultLocks = locks.Where(x => x.Expiration > DateTime.UtcNow).ToList();
                return resultLocks;
            }
        }

        /// <summary>
        /// Saves lock acquired on this file/folder in NTFS stream.
        /// </summary>
        private async Task SaveLockAsync(DateLockInfo lockInfo)
        {
            ...
            {
                //update value
                var existingLock = locks.Single(x => x.LockToken == lockInfo.LockToken);
